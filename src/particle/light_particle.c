...
p_info_t	light_behavior_next(p_info_t prev)
{
	...
	behavior.color[0] = sfColor_fromRGBA(0xFF, 0xE5, 0x26, 200);
	behavior.color[1] = sfColor_fromRGBA(0xFF, 0x85, 0x00, 200);
...
p_info_t	light_behavior(sfVector2f pos)
{
	...
	pos = (sfVector2f){pos.x - 30, pos.y - 60};
	behavior.states = 1;
	...
	behavior.shape = NULL;
	behavior.nb_color = 2;
	behavior = light_behavior_next(behavior);
	return (behavior);
}
